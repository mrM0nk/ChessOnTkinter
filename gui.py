...
#        окно.config(width=ekranX * 9 // 10, height=ekranY * 9 // 10, background="#CFFBA8")
    ...
    def отрисовка_холста(фон_игры, info_panel = False):
        ...
        холст.create_image((ekranX * 9 // 10 - ширина_информационной_панели)//2, ekranY * 9 // 20, image=холст.image, anchor='center')
...
def подсветить_клетки(коорд_свободных_клеток):
    for i in range(len(коорд_свободных_клеток)):
        ...
        холст.create_rectangle(отступ_x + x * клетка - клетка, отступ_y - y * клетка + клетка * 8,
                               отступ_x + x * клетка, отступ_y - y * клетка + клетка * 9, fill=палитра_подсветки[цветовая_схема-1])
    ...
    for i in range(len(короткий_путь)):
        ...
        if номера_ходов:
            холст.create_text(отступ_x - клетка // 2 + клетка * x, отступ_y + клетка * 8 - клетка * y + клетка // 2,
                              font=("Purisa", клетка // 3), text=ch, fill="black")
            time.sleep(1 / 20)
            холст.update()
            ch = ch + 1
