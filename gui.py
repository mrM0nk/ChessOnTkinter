...
def инициализация_интерфейса(info_panel, ask_for_change_settings=False):
    ...
    def отрисовка_холста(фон_игры, info_panel=False):
        ...
        if info_panel:
            ...
            задачи = ["Очистить_поле", "Отрисовать_шахматы", "Подсветить_клетки", "Переместить_коня", "Обойти_доску"]
            принятая_задача = StringVar()
            принятая_задача.set("Очистить_поле")
    ...
    try:
        def motion1(event):
            ...
            холст.create_rectangle(отступ_x + финиш_х * клетка - клетка, отступ_y - финиш_у * клетка + клетка * 8,
                                   ...
                                   width=клетка // 11, outline="Blue", tags="rectBlue")
        ...
        холст.create_rectangle(отступ_x + старт_x * клетка - клетка, отступ_y - старт_y * клетка + клетка * 8,
                               ...
                               outline="Red", tags="rectRed")
        окно.bind('<Button-3>', motion1)
    except:
        pass
