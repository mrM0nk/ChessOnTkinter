...
def отрисовка_шахмат():
    ...
    def отрисовка_черных_шахмат(y=(7, 8)):
        for x in range(8):
            холст.create_image(отступ_x + x * клетка + клетка // 2, отступ_y - y[0] * клетка + клетка * 17 // 2,
                               image=фигуры.get("ПешкаЧ"), tags = "ПешкаЧ{}{}".format(x+1, y))
            time.sleep(задержка_анимации / 3)
            холст.update()
        positions = (4, 3, 2, 0, 1, 2, 3, 4)
        for x in range(0, 8):
            pos = positions[x]
            холст.create_image(отступ_x + x * клетка + клетка // 2, отступ_y - y[1] * клетка + клетка * 17 // 2,
                               image=фигуры.get(список_фигур_черных[pos]), tags=список_фигур_черных[pos] +
                               "{}{}".format(x+1, y))
            time.sleep(задержка_анимации / 2)
            холст.update()

    def отрисовка_белых_шахмат(y=(2, 1)):
        for x in range(8):
            холст.create_image(отступ_x + x * клетка + клетка // 2, отступ_y - y[0] * клетка + клетка * 17 // 2,
                               image=фигуры.get("ПешкаБ"), tags = "ПешкаБ{}{}".format(x+1, y))
            time.sleep(задержка_анимации / 3)
            холст.update()
        positions = (4, 3, 2, 0, 1, 2, 3, 4)
        for x in range(0, 8):
            pos = positions[x]
            холст.create_image(отступ_x + x * клетка + клетка // 2, отступ_y - y[1] * клетка + клетка * 17 // 2,
                               image=фигуры.get(список_фигур_белых[pos]), tags=список_фигур_черных[pos] +
                               "{}{}".format(x+1, y))
            time.sleep(задержка_анимации / 2)
            холст.update()
