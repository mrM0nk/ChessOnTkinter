import gui, ПодсветитьКлетки, КоньAtoB, обход_доски


def btn_click(event):
    try:
        gui.кнопка_старт.unbind('<Button-1>')
        принятая_задача = gui.принятая_задача.get()

        if принятая_задача == "Очистить_поле":
            gui.очистка_доски()

        if принятая_задача == "Отрисовать_шахматы":
            gui.очистка_доски()
            gui.отрисовка_шахмат()

        if принятая_задача == "Подсветить_клетки":
            gui.очистка_доски()
            координата_x, координата_y, фигура = gui.ввод_координат_подсветки()
            gui.нарисовать_фигуру(координата_x, координата_y, фигура)
            коорд_свободных_клеток = ПодсветитьКлетки.свободные_клетки(координата_x, координата_y, фигура)
            gui.подсветить_клетки(коорд_свободных_клеток)

        if принятая_задача == "Переместить_коня":
            gui.очистка_доски()
            старт_x, старт_y, финиш_х, финиш_у = gui.ввод_координат_конюАБ()
            короткий_путь = КоньAtoB.задача_коня(старт_x, старт_y, финиш_х, финиш_у)
            gui.перемести_фигуру(короткий_путь, номера_ходов=False)

        if принятая_задача == "Обойти_доску":
            gui.очистка_доски()
            старт_x, старт_y, фигура = gui.ввод_координат_подсветки()
            пройденные_клетки = обход_доски.обход_доски(старт_x, старт_y)
            gui.перемести_фигуру(пройденные_клетки, номера_ходов=True)

    except:
        pass

    try:
        gui.кнопка_старт.bind('<Button-1>', btn_click)

    except:
        pass


if __name__ == "__main__":
    gui.инициализация_интерфейса(info_panel=True, ask_for_change_settings=False)
    gui.отрисовка_доски()

    gui.кнопка_старт.bind('<Button-1>', btn_click)

    gui.окно.mainloop()